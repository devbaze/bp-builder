...
class ConsoleMakeBuilderScript extends CommandMake
{
    protected $name = 'make:builder-script';
    ...
    protected function getStub()
    {
        ...
        return __DIR__ . "/stubs/builder/templates/$template/Script.js";
    }
    ...
    {
        $index_dir = \get_theme_file_path() . '/resources/scripts/builder/';
        $index_path = $index_dir . 'index.js';
        if (!$this->files->isFile($index_path)) {
            $this->makeIndexJs($index_dir, $index_path);
        }
        ...
        if (strpos($index_js, $import) !== false) {
            return $this->error('Index.js contains Import already');
        }
    ...
    {
        $stub = file_get_contents(__DIR__ . '/stubs/builder/publish/IndexJs.js');
        $this->makeFile($dir, $file, $stub);
        return $this->error('add "import Builder from \'./builder\';" and "Builder.init();" to app.js');
    }
}
